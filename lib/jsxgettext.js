...
function genJade (jadeSources, options) {
  Object.keys(jadeSources).forEach(function (filename) {
    jadeSources[filename] = parseJade(jadeSources[filename], filename, options);
  });
...
function parseJade(str, filename, options) {
  options = options || {};
  ...
  while (parser.peek().type !== 'eos') {
    switch(parser.peek().type) {
      case 'text':
      case 'code':
        token = parser.advance();
        tmp = extractGettext(token.val);
        if (tmp.length) buf.push(tmp + ';');
        break;
      case 'call':
        token = parser.advance();
        ...
        break;
      case 'tag':
        expression = parser.parseTag();

        var script = expression.block.nodes.map(function(node) {
          return node.val;
        }).join("");

        tmp = extractGettext(script);
        if (tmp.length) buf.push(tmp + ';');
        break;
      default:
        parser.advance();
