...
function parseJade(str, options) {
  ...
  do {
    ...
    switch(token.type) {
      case 'attrs':
        var tmp = []
          , attrs = Object.keys(token.attrs);

        // This assumes this is a regular HTML attribute if the type of one of
        // the values of one of the keys in token.attrs is a string,
        if (attrs.length > 0 && typeof token.attrs[attrs[0]] === "string") {
          // The value of following jade code
          //
          //  input(type='text', placeholder='gettext("Placeholder text"))
          //
          // will be be parsed to
          //
          // attrs: {
          //    type: '"text"',
          //    placeholder: '"gettext(\'Placeholder text\')"'
          // }
          ...
          // Jade parses JavaScript inside of a script tag as an attribute.
          // The value of token.attrs is an object, with each key representing
          // the parsed JavaScript code.
          //
          // In order to see if there are any gettext calls in a script tag,
          // first join all of the keys in token.attrs into a string.
          r = extractGettext(attrs.join(" "));
          if (r.length) tmp.push(r);
        }
